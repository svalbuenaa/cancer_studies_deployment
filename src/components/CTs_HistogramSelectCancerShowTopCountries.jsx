...
const CTs_HistogramSelectCancerShowTopCountries = ({ csvPath }) => {
  ...
  // Track window size for responsive behavior (Unchanged)
  useEffect(() => {
    const handleResize = () => {
  ...
  // 💥 FIX 3: Wrap all tick calculations in useMemo.
  const { tickvals, upper } = useMemo(() => {
    ...
    return { tickvals, upper };
  }, [plotData]); // <-- Only recalculate when plotData changes

  // 💥 FIX 1: Stabilize formatCompact function
  ...
  // 💥 FIX 2: Stabilize targetYRange
  // Calculate the target yaxis range for resets (upper bound only)
  const targetYRange = useMemo(
    () => [0, upper * 1.2],
    [upper]
  );

  // Plotly onRelayout handler to force correct axis reset on double-click (Unchanged)
  const handleRelayout = useCallback((event) => {
    // Check if the event is a double-click reset action
    if (event['yaxis.autorange'] === true) {
      // We set the range back to our custom, full range.
  ...
  const baseLayout = useMemo(() => ({
    ...
    yaxis: {
      ...
      tickfont: { color: "black", size: windowWidth <= 1080 ? 9 : 14 },
      // Set the initial range, which includes space for annotations at the top
      range: targetYRange,
      tickmode: "array",
      tickvals: tickvals,
  ...
  }), [selectedCancer, windowWidth, years, targetYRange, tickvals, annotations, formatCompact]);

  // Initialize or update the local layout state when baseLayout changes (Now stable)
  useEffect(() => {
  ...
  return (
    <div className="plotly-responsive-plot-container" style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <Plot
        key={selectedCancer}
        data={plotData}
        layout={layout} // Use the managed layout state
        config={config}
...
export default CTs_HistogramSelectCancerShowTopCountries;