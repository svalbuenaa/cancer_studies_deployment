...
const CTs_ScatterSelectCountryShowCancersIncidenceStudies = ({ csvPath }) => {
  ...
  // 1. State to control the UI revision (used to force reset zoom)
  ...
  // 2. Reset Handler (for both double-click and custom button)
  const handleReset = () => {
    setUiRevision(prev => prev + 1);
  };

  // 3. Configuration with Custom Reset Button
  const config = useMemo(() => ({
    responsive: true,
    displaylogo: false,
    doubleClick: false, // Disable native double click
    
    // Remove native reset button and others
    modeBarButtonsToRemove: ["zoom2d","pan2d","select2d","lasso2d","zoomIn2d","zoomOut2d","autoScale2d","resetScale2d","hoverClosestCartesian","hoverCompareCartesian","toImage"],
    ...
    modeBarButtonsToAdd: [{
      ...
      icon: {
         ...
         // The path defines the house shape
         path: "M250 450 L50 250 L100 250 L100 50 L400 50 L400 250 L450 250 Z",
         // FIX: Flip icon vertically and translate it back down (500 is height)
         transform: 'matrix(1 0 0 -1 0 500)' 
      },
      click: handleReset 
    }]
  }), [handleReset]); // Dependency on handleReset ensures click function is fresh
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: "100%" }}>
      ...
      <div style={{ width: plotSize, height: plotSize }}>
        <Plot
          ...
          onDoubleClick={handleReset} // Custom double-click handler
          layout={{
            uirevision: uiRevision, // 4. Ties layout to the reset state
            xaxis: { 
              ...
              ticksuffix: "%", 
              // Ranges remain unchanged as requested
