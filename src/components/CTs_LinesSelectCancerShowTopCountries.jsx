...
const CTs_LinesSelectCancerShowTopCountries = ({ csvPath }) => {
  ...
  // 💥 FIX 1 & 2: Stabilize maxVal and xRange calculations
  const { maxVal, xRange } = useMemo(() => {
    const currentMaxVal = Math.max(...plotData.flatMap(t => t.y), 0);
    const yearNumbers = years.map(y => parseInt(y, 10));
    const currentXRange = yearNumbers.length > 0
      ? [yearNumbers[0] - 0.5, yearNumbers[yearNumbers.length - 1] + 0.5]
      : [0, 1]; // Provide a safe default
    return { maxVal: currentMaxVal, xRange: currentXRange };
  }, [plotData, years]);


  // 💥 FIX 3: Stabilize targetYRange calculation
  ...
  const handleRelayout = useCallback((event) => {
      ...
      if (event['yaxis.autorange'] === true) {
          setLayout(prevLayout => ({
              ...prevLayout,
              // Use targetYRange to reset to the desired full view
  ...
  const baseLayout = useMemo(() => ({
    ...
    yaxis: {
      ...
      tickfont: { color: "black", size: windowWidth <= 1080 ? 9 : 14 },
      // Set the initial range, which includes space above the maximum value
      range: targetYRange,
  ...
  }), [selectedCancer, windowWidth, years, xRange, targetYRange]);

  // Initialize or update the local layout state when baseLayout changes
  useEffect(() => {
